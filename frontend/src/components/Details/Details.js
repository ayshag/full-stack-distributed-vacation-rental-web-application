...
class Details extends Component {
    ...
    submitBook = (e) => {
        ...
        e.preventDefault();
	console.log("User at submitBook: ", this.props.username);
        const data = {
	...
	if (!this.props.authFlag) {
        ...
        if (this.props.propertydetails) {
            ...
            if (this.props.propertydetails.bedrooms)
                bedrooms = this.props.propertydetails.bedrooms;

            if (this.props.propertydetails.bathrooms)
                bathrooms = this.props.propertydetails.bathrooms;

            if (this.props.propertydetails.sleeps)
                sleeps = this.props.propertydetails.sleeps;
...
const mapStateToProps = state => {
     console.log("User at MSTP: ", state.reducer.username);
    return {
...
const mapDispatchStateToProps = dispatch => {
    return {
        ...
        getdetails: (data) => {
            axios.get('http://localhost:3001/details', data, {headers: {authorization : "jwt" + sessionStorage.getItem("usertoken")}})
                .then((response) => {
        ...
        bookProperty: (values) => {
console.log("User at bookProperty: ", values.user);
